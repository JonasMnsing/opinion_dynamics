...
def run_trajectory(N, graph_fn, p_noise, max_steps=100_000):
    A = graph_fn(N)
    opinions = initialize_opinions(N)
    m_list, rho_list = [], []
    for t in range(max_steps):
        opinions = voter_interaction(opinions, A, p_noise)
        m_list.append(average_opinion(opinions))
        rho_list.append(interface_density(opinions, A))
        if abs(m_list[-1]) == 1.0:
            break
